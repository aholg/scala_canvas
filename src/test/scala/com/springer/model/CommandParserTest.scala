...
class CommandParserTest extends FunSuite with Matchers {
  ...
  test("It parses input and returns successful commands") {
    ...
    CommandParser.parseInput("L 1 1 4 1") shouldBe Line(0, 0, 3, 0)
    CommandParser.parseInput("R 1 1 4 4") shouldBe Rectangle(0, 0, 3, 3)
    CommandParser.parseInput("B 1 1 K") shouldBe BucketFill(0, 0, 'K')
